...
spacex_df = pd.read_csv("/Users/jessicaarmitage/Desktop/DataScience/SpaceX/spacex_launch_dash.csv")
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Total Success Count for all launch sites')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df=spacex_df[spacex_df['Launch Site']== entered_site]
        filtered_df=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig=px.pie(filtered_df,values='class count',names='class',title="Percentage of Successful Launches for "+ entered_site)
        return fig
# TASK 4:
